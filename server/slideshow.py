...
Downloads images + audio, renders 720x1280 portrait video with 4s per image,
libx264 ultrafast stillimage CRF 28. Streams the output file (does NOT load
entire MP4 into memory) for O(1) memory usage regardless of video size.

Temp files are cleaned up via a background task after streaming completes,
with a safety net to clean on crash.
"""
...
async def _download_to_file(
    ...
    if not resp.is_success:
        ...
        raise SlideshowError(
            f"Failed to download {url[:60]}...: HTTP {status}",
...
async def render_slideshow(
    ...
    """Render slideshow and return {output_path, temp_dir, file_size}.

    Caller is responsible for cleanup via cleanup_temp(temp_dir) after
    streaming the output file is complete.
    """
    if not photo_urls:
        raise SlideshowError("No photos available for slideshow", 400)

    ffmpeg = ffmpeg_path or shutil.which("ffmpeg") or os.environ.get("FFMPEG_PATH", "ffmpeg")
    ...
    if audio_path and os.path.exists(audio_path):
        ...
        args.extend(
            ["-filter_complex", ";".join(filter_parts), "-map", "[vout]",
             ...
             "-c:v", "libx264", "-preset", "ultrafast", "-tune", "stillimage",
             "-crf", "28", "-b:v", "320k", "-maxrate", "360k", "-bufsize", "720k",
             "-threads", "1", "-max_muxing_queue_size", "1024", output_path]
        )
    ...
    try:
        _, stderr_data = await asyncio.wait_for(proc.communicate(), timeout=FFMPEG_TIMEOUT_SECONDS)
    ...
    if proc.returncode != 0:
        err = stderr_data.decode("utf-8", errors="replace")[-400:] if stderr_data else "unknown"
        cleanup_temp(temp_dir)
...
def open_file_stream(path: str):
    """Open file for streaming. Caller must close the handle."""
    return open(path, "rb")
