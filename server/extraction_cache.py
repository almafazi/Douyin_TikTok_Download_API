"""Extraction cache for POST /tiktok raw crawler results.

Caches raw crawler output to avoid repeated upstream calls.
TTL 1800s (30 menit). Stampede protection via lock + polling.
Optional Redis backend with graceful fallback to memory.

Cache key: SHA-256(url|proxy|impersonate|version), prefix exinfo:
Lock key:   prefix exlock:
"""
...
def _build_raw_key(url: str, options: Dict[str, Any]) -> str:
    return f"{url}|{options.get('proxy') or ''}|{options.get('impersonate') or ''}|{options.get('version') or ''}"
...
async def _get_backend() -> CacheBackend:
    ...
    async with _lock:
        ...
        if REDIS_URL:
            ...
            _backend = MemoryCacheBackend()
            _use_redis = False
            logger.info("Using in-memory backend (set REDIS_URL to enable Redis)")
        return _backend
...
async def get_or_extract(
    ...
    try:
        ...
        if cached:
            logger.debug("cache hit: %s", ckey[:16])
    ...
    try:
        ...
        try:
            await backend.set(ckey, json.dumps(raw), EXTRACT_CACHE_TTL)
        except Exception as exc:
...
async def invalidate_extract_cache(url: str, options: Dict[str, Any]) -> None:
    if _backend is None:
        return
    try:
        await _backend.delete(_build_cache_key(url, options))
    except Exception:
        pass
