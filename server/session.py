...
In-memory backend with optional Redis. TTL 300s (5 menit).
Key prefix: tiktok:session:

Production hardening:
- Redis SCAN (not KEYS) for count()
- Redis fallback to memory on connection failure mid-request
- Proper logging (not print)
- asyncio.get_running_loop() (not deprecated get_event_loop)
- ping() health check for /health endpoint
"""
...
REDIS_KEY_PREFIX = "tiktok:session:"
...
class RedisBackend(SessionBackend):
    ...
    async def count(self) -> int:
        ...
        async for _ in self._client.scan_iter(
            match=f"{REDIS_KEY_PREFIX}*", count=200
        ):
            count += 1
        return count
        ...
        if REDIS_URL:
            try:
                ...
                if await rb.ping():
                    ...
                    logger.info("Using Redis backend: %s", _mask_url(REDIS_URL))
                else:
